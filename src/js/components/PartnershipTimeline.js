...
export default function PartnershipTimeline(_innings,options) {
	...
	//var xscale=d3.scale.linear().range([0,WIDTH-(margins.left+margins.right)]).domain([0,match.overall.total_balls]),
	//	yscale=d3.scale.linear().range([HEIGHT-(margins.top+margins.bottom),0]).domain([0,d3.max(match.score,function(d){return d.values.runs;})]);
	...
	var partnershipTimeline=d3.select(options.container)
					.append("div")
	...
	*/
	...
	function highlightProfile(partnership) {
		...
		if(partnership.id2 && partnership.id2!="*") {
			...
			tooltipPartnership.show([
				...
				d3.mean([partnership.order1,partnership.order2])*BAR,
	...
	var svg=innings.append("svg")
							.attr("width",function(d){
								...
								var last_player=options.innings.values.total_balls;
	...
	var players=svg.append("g")
					.attr("class","players "+options.getTeamInfo(options.innings.values.id).code)
					.attr("transform","translate("+margins.left+","+0+")");
	...
	innings=all_innings.selectAll("g.innings")
				...
				.enter()
					.append("g")
						...
						.attr("transform",function(d){
							var x=xscale(d.values.starting_balls),
								y=0;
							x=0;
							return "translate("+x+","+y+")";
						})
	...
	partners.append("rect")
				...
				.attr("y",function(d){
					return (Math.min(d.order1,d.order2)-1)*BAR + BAR/2 +3;//0;//(d.Wkt)*27 - 27/2 + 6 - 1.5
				})
				.attr("width",function(d){
