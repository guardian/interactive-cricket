...
export default function Partnerships(data,options) {
	...
	let _updateData = (data) => {
		...
		this.match.innings=d3.nest()
				...
				.rollup(function(leaves){
					...
					let stuff= {
						...
						total_balls:options.max_overs?(options.max_overs*6):(d3.sum(leaves,(d)=>{
													return d.overs*6;
												})),
						innings:(((leaves) => {
							let starting_over=0,
		...
		this.match.overall={
			...
			total_balls__:d3.sum(this.data.partnerships,(d) => {
				return d.overs*6;
			}),
			total_balls:options.max_overs?(options.max_overs*6*2):(d3.sum(leaves,(d)=>{
													return d.overs*6;
												})),
	...
	let _buildChart = () => {
		...
		let xscale=d3.scale.linear().range([0,WIDTH-(margins.left+margins.right)]).domain([0,this.match.overall.total_balls]),
			yscale=d3.scale.linear().range([HEIGHT-(margins.top+margins.bottom),0]).domain([0,d3.max([options.min_runs,d3.max(this.match.score,(d) => (d.values.runs))])]);

		let runscale=d3.scale.linear().domain([0,this.match.overall.runs]);

		xscale.range([0,WIDTH-(60+40)*2])
					...
					.append("div")
						...
						.each(function(d,i) {
							...
							partnershipChart=new PartnershipsChart(d,{
								...
								yscale:yscale,
								margins:margins,
								target:i>0?(self.match.innings[i-1].values.total_runs+1):null,
								height:HEIGHT,
